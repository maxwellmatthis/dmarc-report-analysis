...
try:
    if (not os.path.isfile(DATA_STORE)):
        ...
        json.dump({'lastChecked': str(int(time()) - 7889238)},
                  dataStoreHandler)
...
def get(batch_size):
    try:
        ...
        messages = service.users().messages().list(userId='me', q='after:' +
                                                   getLastChecked(), maxResults=batch_size).execute()
        print('checking ' + (str(len(messages['messages']))
                             if 'messages' in messages else '0') + ' new messages')
            ...
            for message in messages['messages']:
                ...
                if ('parts' in msg['payload']):
                    ...
                    for part in msg['payload']['parts']:
                        if (part['filename'].split('.')[-1] in ['gz', 'zip']):
                            ...
                            service.users().messages().modify(
                                userId='me', id=message['id'], removeLabelIds=['UNREAD'])
            checkedNow()
